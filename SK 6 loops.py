...
ax1.plot(isoDIC_wwc["d18_CO3"]-isoDIC_wwc["d18_CO3"].iloc[0], isoDIC_wwc["D17_CO3"]-isoDIC_wwc["D17_CO3"].iloc[0],
         c="darkred", ls="solid", zorder=1, lw=1.5, label=r"pH = 8.5, $\it{T}$ = 27 °C")
ax1.plot(isoDIC_cwc["d18_CO3"]-isoDIC_cwc["d18_CO3"].iloc[0], isoDIC_cwc["D17_CO3"]-isoDIC_cwc["D17_CO3"].iloc[0],
         c="darkblue", ls="solid", zorder=1, lw=1.5, label=r"pH = 8.8, $\it{T}$ = 9 °C")
ax1.plot(isoDIC_wwc2["d18_CO3"]-isoDIC_wwc2["d18_CO3"].iloc[0], isoDIC_wwc2["D17_CO3"]-isoDIC_wwc2["D17_CO3"].iloc[0],
         c="hotpink", ls="solid", zorder=2, lw=1, label=r"pH = 8.8, $\it{T}$ = 27 °C")
ax1.plot(isoDIC_cwc2["d18_CO3"]-isoDIC_cwc2["d18_CO3"].iloc[0], isoDIC_cwc2["D17_CO3"]-isoDIC_cwc2["D17_CO3"].iloc[0],
...
# Add the arrow markers
ax2.annotate("",
             (isoDIC_wwc["D64_CO3"].iloc[wwc_target]-isoDIC_wwc["D64_CO3"].iloc[0],
              isoDIC_wwc["D63_CO3"].iloc[wwc_target]-isoDIC_wwc["D63_CO3"].iloc[0]),
             (isoDIC_wwc["D64_CO3"].iloc[wwc_target-1]-isoDIC_wwc["D64_CO3"].iloc[0],
              isoDIC_wwc["D63_CO3"].iloc[wwc_target-1]-isoDIC_wwc["D63_CO3"].iloc[0]),
             ha="center", va="center", zorder=0,
             arrowprops=dict(arrowstyle="->", color="darkred", lw=1.5))
ax2.annotate("",
             (isoDIC_cwc["D64_CO3"].iloc[cwc_target]-isoDIC_cwc["D64_CO3"].iloc[0],
              isoDIC_cwc["D63_CO3"].iloc[cwc_target]-isoDIC_cwc["D63_CO3"].iloc[0]),
             (isoDIC_cwc["D64_CO3"].iloc[cwc_target-1]-isoDIC_cwc["D64_CO3"].iloc[0],
              isoDIC_cwc["D63_CO3"].iloc[cwc_target-1]-isoDIC_cwc["D63_CO3"].iloc[0]),
             ha="center", va="center", zorder=0,
             arrowprops=dict(arrowstyle="->", color="darkblue", lw=1.5))
ax2.annotate("",
             (isoDIC_wwc2["D64_CO3"].iloc[wwc2_target]-isoDIC_wwc2["D64_CO3"].iloc[0],
              isoDIC_wwc2["D63_CO3"].iloc[wwc2_target]-isoDIC_wwc2["D63_CO3"].iloc[0]),
             (isoDIC_wwc2["D64_CO3"].iloc[wwc2_target-1]-isoDIC_wwc2["D64_CO3"].iloc[0],
              isoDIC_wwc2["D63_CO3"].iloc[wwc2_target-1]-isoDIC_wwc2["D63_CO3"].iloc[0]),
             ha="center", va="center", zorder=2,
             arrowprops=dict(arrowstyle="->", color="hotpink", lw=1))
ax2.annotate("",
             (isoDIC_cwc2["D64_CO3"].iloc[cwc2_target]-isoDIC_cwc2["D64_CO3"].iloc[0],
              isoDIC_cwc2["D63_CO3"].iloc[cwc2_target]-isoDIC_cwc2["D63_CO3"].iloc[0]),
             (isoDIC_cwc2["D64_CO3"].iloc[cwc2_target-1]-isoDIC_cwc2["D64_CO3"].iloc[0],
              isoDIC_cwc2["D63_CO3"].iloc[cwc2_target-1]-isoDIC_cwc2["D63_CO3"].iloc[0]),
             ha="center", va="center", zorder=2,
             arrowprops=dict(arrowstyle="->", color="cornflowerblue", lw=1))
